...
| maxConcurrentDeleteJobs | int | `2` | Cap on concurrently running Snapshot-delete BATCH mover Jobs, across every repository (mass-deletion protection). Bounds how much delete traffic a mass-deletion wave (or an incident's retroactive prune) can put on the backend at once; it does NOT bound how many Snapshots one batch Job deletes, nor does it gate whether a deletion is allowed at all — that's deletionProtection.threshold on the Repository/ClusterRepository. Must be >= 1: a zero cap would wedge every deletion cluster-wide. |
