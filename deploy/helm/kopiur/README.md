...
| controller.listenAddr | string | `"[::]:8081"` | Address the controller's HTTP server binds to (env KOPIUR_HTTP_ADDR; serves /metrics, /healthz, /readyz). Defaults to the dual-stack wildcard "[::]:8081", which serves both IPv4 and IPv6 kubelets (a wildcard IPv6 bind also accepts IPv4 on Linux when net.ipv6.bindv6only=0, the default). Override to "0.0.0.0:8081" only where IPv6 is disabled in the pod network namespace, where a "[::]" bind fails outright. Must agree on port with probePort below (the Service and probes target that port, not whatever this address happens to contain). |
| controller.livenessProbe | object | `{"httpGet":{"path":"/healthz","port":"metrics"},"initialDelaySeconds":10,"periodSeconds":15}` | Liveness probe for the controller container. The whole probe is passed through with `toYaml`, so you can retune timings/thresholds or swap the scheme entirely; set to `{}` to drop the probe. `port: metrics` is the named container port (probePort above). |
...
| controller.readinessProbe | object | `{"httpGet":{"path":"/readyz","port":"metrics"},"initialDelaySeconds":5,"periodSeconds":10}` | Readiness probe for the controller container (same passthrough as livenessProbe; set to `{}` to drop it). |
...
| webhook.extraEnv | list | `[]` | Extra environment variables for the webhook container (list of `{name, value}` / `{name, valueFrom}` entries), appended after the operator-managed env. Mirrors `controller.extraEnv`. |
...
| webhook.listenAddr | string | `"[::]:8443"` | Address the webhook server binds to (env KOPIUR_WEBHOOK_ADDR). Defaults to the dual-stack wildcard "[::]:8443" (serves both IPv4 and IPv6 API servers); override to "0.0.0.0:8443" only where IPv6 is disabled in the pod network namespace. |
