...
![Version](https://img.shields.io/static/v1?label=Version&message=0.10.10&color=informational&style=flat-square) <!-- x-release-please-version -->
![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square)
![AppVersion](https://img.shields.io/static/v1?label=AppVersion&message=0.10.10&color=informational&style=flat-square) <!-- x-release-please-version -->
...
| maxConcurrentDeleteJobs | int | `0` | 0 = no cluster-wide cap (default); set to bound concurrent snapshot-delete batch Jobs across ALL repositories. Each repository is already capped on its own by spec.concurrency.maxConcurrentDeleteJobs (default 1: one batch at a time, each deleting hundreds of Snapshots in one kopia call), so this is an opt-in backstop for a resource-constrained cluster, not a load-bearing safety mechanism: a small cap risks head-of-line-blocking every OTHER repository's deletions behind one slow/failing one. It does NOT bound how many Snapshots one batch Job deletes, nor does it gate whether a deletion is allowed at all — that's deletionProtection.threshold on the Repository/ClusterRepository. |
...
| rbacNamespaceReadForStreamSources | bool | `false` | Under `installScope: namespaced`, also grant the operator cluster-scoped `get` on `namespaces` (and nothing else) via a small supplementary ClusterRole.  ONLY needed for `stream` sources. A stream source execs a command inside a workload pod, which requires the operator to mint `pods/exec` for the mover — arbitrary code execution in every pod in that namespace, on a ServiceAccount that outlives the Job. Kopiur therefore refuses unless a cluster admin has annotated the namespace `kopiur.home-operations.com/stream-exec-movers=true`, and it FAILS CLOSED when it cannot read the Namespace to check (unlike the elevated-mover opt-in, which fails open — `pods/exec` is the larger grant, and a 403 on `namespaces get` cannot be distinguished from a deny an admin added on purpose).  A namespaced install has no `namespaces` rule at all, so stream sources are refused permanently there. Set this to `true` to add just that one read, or use `installScope: cluster` (which already has it). Leave it `false` if you do not use stream sources: PVC and NFS sources need nothing here.  Ignored under `installScope: cluster`. |
