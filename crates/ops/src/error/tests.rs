...
fn not_a_directory_points_at_cat_download() {
    let msg = OpsError::NotADirectory {
        path: "a.txt".into(),
        entry_type: "f".into(),
    }
    .to_string();
...
fn mover_image_resolution_failures_carry_the_fix() {
    let msg = OpsError::MoverImageUnresolvable {
...
fn cross_namespace_repo_session_names_the_gc_hazard_and_local_escape() {
    let msg = OpsError::RepoOutsideSessionNamespace {
...
fn snapshot_not_browsable_names_the_status_field_and_fix() {
    let msg = OpsError::SnapshotNotBrowsable {
        name: "db-1".into(),
...
fn repository_underivable_explains_both_derivation_sources() {
    let msg = OpsError::RepositoryUnderivable {
        snapshot: "db-1".into(),
    }
    .to_string();
...
fn cross_namespace_creds_offer_three_outs() {
    let msg = OpsError::CredsOutsideSessionNamespace {
...
fn cluster_repo_secret_without_namespace_names_the_field() {
    let msg = OpsError::ClusterRepoSecretNamespaceMissing {
        secret: "creds".into(),
        repository: "nas".into(),
    }
    .to_string();
...
fn operator_namespace_unresolvable_names_the_configmap_and_the_discovery_rule() {
    let msg = OpsError::OperatorNamespaceUnresolvable {
        repository: "nas".into(),
...
fn ca_bundle_unresolvable_names_the_cause_and_the_fix() {
    let msg = OpsError::CaBundleUnresolvable {
        repository: "nas".into(),
...
fn session_pod_failure_and_timeout_point_at_pods_and_doctor() {
    let failed = OpsError::SessionPodFailed {
    ...
    let timeout = OpsError::SessionNotReady {
        job: "kopiur-browse-nas-abc123".into(),
        after: "2m".into(),
    }
    .to_string();
...
fn path_errors_teach_the_path_grammar() {
    let invalid = OpsError::InvalidPath {
    ...
    let missing = OpsError::PathNotFound {
    ...
    let dir = OpsError::IsADirectory { path: "sub".into() }.to_string();
    assert!(dir.contains("is a directory"), "{dir}");
    assert!(dir.contains("ls <snapshot> sub"), "{dir}");

    let link = OpsError::NotAFile {
        path: "link".into(),
        entry_type: "s".into(),
    }
    .to_string();
...
#[test]
fn missing_catalog_entry_is_actionable() {
    let gone = OpsError::SnapshotMissingInRepo { id: "kdead".into() }.to_string();
...
fn unexpected_kopia_output_is_reported_as_a_compat_bug() {
    let msg = OpsError::UnexpectedKopiaOutput {
...
fn stream_io_names_what_was_being_copied() {
    ...
    .to_string();
    assert!(
        msg.contains("I/O error while streaming sub/b.txt from the session pod"),
        "{msg}"
    );
...
/// One representative variant per [`OpsErrorKind`], so a status-code mapping
/// built on `kind()` can be trusted. `kind()` itself is an exhaustive `match`,
/// so a new variant cannot compile without picking a class.
#[test]
fn kind_classifies_one_representative_of_every_class() {
    let cases: Vec<(OpsError, OpsErrorKind)> = vec![
        (
            OpsError::Forbidden {
                verb: "patch",
                resource: "snapshots",
        ...
        (
            OpsError::InvalidPath {
                path: "..".into(),
                reason: "escapes the root".into(),
            },
            OpsErrorKind::Invalid,
        ),
        (
            OpsError::WaitTimeout {
                what: "Snapshot db-1".into(),
                after: "30m".into(),
                hint: "watch it".into(),
            },
        ...
        (
            OpsError::Serialization {
                what: "a Snapshot",
                source: std::io::Error::other("bad").into(),
            },
