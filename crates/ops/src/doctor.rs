//! Health diagnosis: the checks behind `kubectl kopiur doctor` — CRDs,
//! operator and webhook health (including a live admission probe), repository
//! readiness, credential Secrets, blocked/stuck work, recent failures, and
//! recent warnings. Every check lands in a closed enum and produces Pass /
//! Warn / Fail{what, why, fix}; [`DoctorReport::exit_code`] is 1 iff anything
//! failed. RBAC the caller lacks degrades a check to Warn (with the missing
//! grant named), never a crash.
//!
//! Rendering is the caller's job: the CLI prints a text report, a server
//! serializes [`DoctorReport`] as-is.
//!
...
//! grows is one this layer reports with no code change on this side.
...
async fn check_crds(ctx: &OpsCtx) -> Outcome {
...
/// probe gates on existence — a present-but-unready webhook should be probed,
/// since that is exactly when "failed calling webhook" surfaces).
async fn check_deployment(
    ...
    let api: Api<Deployment> = match operator_namespace {
...
/// (server-side dryRun).
async fn check_webhook_admission(ctx: &OpsCtx, webhook_installed: bool) -> Outcome {
...
/// The facts about one `Repository`/`ClusterRepository` the repository-scoped
/// checks read, flattened so both kinds go through one code path.
pub struct RepoSummary {
    /// Which of the two repository kinds this is.
    pub kind: RepositoryKind,
    /// The object's name.
    pub name: String,
    /// Its namespace; `None` for a cluster-scoped `ClusterRepository`.
    pub namespace: Option<String>,
    /// `status.phase`, as its label string; `None` when no status was written.
    pub phase: Option<String>,
    /// The `Ready` condition's message, quoted verbatim in every report line.
    ...
    /// `spec.backend`, for resolving the credential Secret references.
    pub backend: kopiur_api::Backend,
    /// `spec.encryption`, for the same reason.
    pub encryption: kopiur_api::common::Encryption,
}
...
async fn check_credentials(ctx: &OpsCtx, repos: &[RepoSummary]) -> Outcome {
    let mut missing = Vec::new();
...
pub fn evaluate_snapshot_replications(
...
pub struct Work {
    /// Every `Snapshot` in scope.
    pub snapshots: Vec<Snapshot>,
    /// Every `Restore` in scope.
    pub restores: Vec<Restore>,
    /// Every `SnapshotSchedule` in scope.
    pub schedules: Vec<SnapshotSchedule>,
    /// Every `SnapshotPolicy` in scope.
...
/// What a doctor run needs from its caller: the two windows the age-based
/// verdicts use, plus where the operator runs when the caller already knows.
#[derive(Debug, Clone)]
pub struct DoctorParams {
    /// Treat a Snapshot/Restore non-terminal for longer than this as stuck.
    /// Only the AGE-based verdict uses it — an object parked on a structural
    /// gate is reported immediately, however young it is.
    pub stuck_threshold: std::time::Duration,
    /// How far back a terminal `Failed` Snapshot/Restore counts as a CURRENT
    /// problem (Fail) rather than retained history (Warn).
    pub failure_lookback: std::time::Duration,
    /// The namespace the operator runs in, when the caller already knows it
