...
pub fn translate_source(
    ...
    // change independently of what this fork source actually needs; it defaults to
    ...
    // fork-era behavior regardless of kopiur's current default.
    let raw_copy_method = kopia.copy_method.as_deref();
...
mod tests {
    ...
    fn source_path_and_identity_overrides_carry_verbatim() {
        ...
        // No spec.kopia.copyMethod given — must still emit an explicit Direct rather
        // than leaving the field absent (which would pick up kopiur's own default).
        assert_eq!(policy["spec"]["copyMethod"], "Direct");
    }

    #[test]
    fn unknown_copy_method_translates_to_explicit_direct_and_is_unmappable() {
        // An unrecognized copyMethod string still must not leave the field absent — it
        // falls back to an explicit Direct, and the accounting message must not lie
        // about "defaulting to kopiur's Direct" (kopiur's default is Snapshot now).
        let spec = source_spec(serde_json::json!({
            "sourcePVC": "data",
            "kopia": { "repository": "vs-secret", "copyMethod": "Wat" }
        }));
        let t = translate_source("app", "media", &spec, &repo_ref()).unwrap();
        let policy = &t.objects[0];
        assert_eq!(policy["spec"]["copyMethod"], "Direct");
        ...
        let note = t
            .notes
            .iter()
            .find(|n| n.field == "spec.kopia.copyMethod")
