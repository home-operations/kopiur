...
use crate::cache::authz::SarCache;
...
macro_rules! namespaced_kinds {
    ($($ty:ty => { kind: $kind:literal, plural: $plural:literal, store: $field:ident }),+ $(,)?) => {
        $(
...
namespaced_kinds! {
    Repository => { kind: "Repository", plural: "repositories", store: repositories },
    SnapshotPolicy => { kind: "SnapshotPolicy", plural: "snapshotpolicies", store: policies },
    Snapshot => { kind: "Snapshot", plural: "snapshots", store: snapshots },
    SnapshotSchedule => { kind: "SnapshotSchedule", plural: "snapshotschedules", store: schedules },
    Restore => { kind: "Restore", plural: "restores", store: restores },
    Maintenance => { kind: "Maintenance", plural: "maintenances", store: maintenances },
    RepositoryReplication => { kind: "RepositoryReplication", plural: "repositoryreplications", store: repository_replications },
    SnapshotReplication => { kind: "SnapshotReplication", plural: "snapshotreplications", store: snapshot_replications },
}

cluster_scoped_kinds! {
    ClusterRepository => { kind: "ClusterRepository", plural: "clusterrepositories", store: cluster_repositories },
}
...
impl Source {
    ...
    /// # Errors
    ///
    /// [`OpsError`] if the apiserver refused or could not answer. In
    /// `Impersonated` mode a caller with no access gets the apiserver's own 403;
    /// in `Cache` mode they get an empty list, because a list the caller may not
    /// perform anywhere yields nothing to show rather than an error — the same
    /// thing `kubectl get -A` shows for a namespace-scoped user.
    pub async fn list<K: KopiurKind>(
        ...
        match self {
            Self::Cache { stores, sar } => {
                ...
                // object. For a cluster-scoped kind that set is empty, so the
                // cluster-wide probe inside `visible_namespaces` is the only
                // question asked — and the only one that can return anything.
                let candidates: BTreeSet<String> = match namespace {
                    Some(ns) => std::iter::once(ns.to_owned()).collect(),
                    None => stores.namespaces_present::<K>(),
                };
        ...
        match self {
            Self::Cache { stores, sar } => {
                if !sar
                    ...
                    .await?
                {
                    return Err(forbidden::<K>("get", namespace, name));
                }
...
fn forbidden<K: KopiurKind>(verb: &'static str, namespace: Option<&str>, name: &str) -> OpsError {
    let status = kube::core::Status::failure(
        &format!(
            "{plural}.{group} {name:?} is forbidden: User cannot {verb} resource \
...
mod tests {
    ...
    fn a_denied_cache_get_looks_exactly_like_an_apiserver_403() {
        let err = forbidden::<Snapshot>("get", Some("prod"), "nightly");
    ...
    fn a_cluster_scoped_denial_names_no_namespace() {
        let rendered = forbidden::<ClusterRepository>("get", None, "central").to_string();
