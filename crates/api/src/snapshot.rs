...
pub struct SnapshotSpec {
    ...
    /// - `mover.cache` here changes only this run's kopia cache **budgets**
    ///   (`--content-cache-size-mb`/`--metadata-cache-size-mb`) and the ephemeral
    ///   cache volume. It never resizes or re-classes a `cache.mode: Persistent`
    ///   cache PVC: that PVC is named per-POLICY and shared by every sibling
    ///   `Snapshot`, so a per-run override must not mutate state the siblings
    ///   depend on.
