...
#[test]
fn rules_a_and_b_are_at_their_paid_down_floor() {
    // The #359 paydown left exactly two Rule A sites (single-variant `Unknown`
    // probes) and zero Rule B sites. This is a floor, not a snapshot: it fails
    // if the population GROWS, and it fails if an exemption is deleted without
    // deleting the code, which is what keeps the paydown from silently eroding.
    let findings = collect().expect("workspace sources");
    let a = findings
        .iter()
        .filter(|f| f.rule == Rule::NonExhaustiveMatches)
        .count();
    let b = findings
        .iter()
        .filter(|f| f.rule == Rule::WildcardArm)
        .count();
    assert_eq!(
        b, 0,
        "a `_ =>` arm over a phase enum came back: {findings:#?}"
    );
    assert!(
        a <= 2,
        "Rule A grew past the two reviewed `Unknown` probes: {findings:#?}"
    );
}
