...
| `sourcePathOverride` | string | —<br><sub>maxLength 4096</sub> | What kopia records as the source path (default `/pvc/&lt;name&gt;`, or the NFS export `path`).<br>Refused on a `pvcSelector` source: it is one literal path, so it would collapse every matched PVC onto a single kopia source and merge their histories into one stream. Use `sourcePathStrategy`, which derives a distinct path per PVC, or put the override on its own `pvc:` source. |
