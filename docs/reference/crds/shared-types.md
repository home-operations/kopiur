...
- **`retain`** is how many discovered `Snapshot` objects stay materialized, which is what bounds the etcd footprint on a large repository. `perIdentity` keeps the most recent N per `username@hostname:path` identity; snapshots this cluster produced do not count toward it, and `0` turns discovered-snapshot materialization off entirely. `maxAgeDays` expires discovered objects older than N days, with a minimum of 1. Both limits apply to the discovered objects that **already exist** as well as to newly listed snapshots, so tightening `retain` reclaims a backlog on the next scan (when coverage is `Partial`, only `maxAgeDays` applies to objects outside the materialization window). Independently of `retain`, one scan materializes at most a window of 1,000 snapshots (fewer if the scan result would exceed its size budget), filled newest-first and round-robin across identities; `status.catalog.coverage` says whether the last scan's listing was `Complete`, `Capped` to that window, or `Partial`. See [The catalog](../../repositories.md#the-catalog--discovered-snapshots).
