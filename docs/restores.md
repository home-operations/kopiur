...
Rule 2 runs before rule 3 for a related reason. A policy may list several plain `pvc:` sources, and **today only the first one is actually backed up** — a policy with no `pvcSelector` is never fanned out, so `sources: [{pvc: {name: a}}, {pvc: {name: b}}]` captures `/pvc/a` and nothing else. (That is a pre-existing backup-side limitation, tracked separately.) Reading `sources[0]`'s path for *every* target therefore returned a path that was real but belonged to another volume: restoring into PVC `b` silently filled it with `a`'s data. With rule 2 first, `b` resolves `/pvc/b` — a path the repository has never seen — so you get an honest `SnapshotNotFound`, or an empty volume under `Continue`, instead of the wrong data under a green `Completed`. A target that matches **no** plain source still falls back to `sources[0]` (rule 3) — but **only for a selector-free policy**. That is what keeps "restore this plain-`pvc:` policy's data into a differently-named scratch volume" working. A policy with a `pvcSelector` never reaches rule 3: rule 4 derives the path from the *target's* name, so a differently-named target derives a path the repository has never seen (see the upgrade note below). Name the member you want with `sourcePath` whenever the target is not the volume that was backed up.
...
This also changes three previously-working shapes on upgrade:

- **A selector-policy member restored into a differently-named PVC** (a direct `target.pvc`/`pvcRef` — or a populator claim — whose name is not the member's). It used to resolve a pathless identity, which matched the newest snapshot of *any* member; it now derives `/pvc/<target-name>` (rule 4), a path the repository has never seen, so the restore reports `SnapshotNotFound` — or, under `Continue`, pins `NoSnapshot` and provisions an **empty** volume with `Resolved=True reason=NoSnapshotContinue`. The message names the derived path and the fix: set `fromPolicy.sourcePath: /pvc/<member>` (`kubectl kopiur restore --from-policy <p> --source-path /pvc/<member> …`). A `Continue` decision is *pinned*, so create a new `Restore` (or re-create the claiming PVC) once you have set it.
- **A mixed policy** (a plain `pvc:` source plus a selector) restoring cross-namespace used to fall back to the plain source's path; it now derives from the target instead.
- **A policy with several plain sources** (or an `nfs` source ahead of a `pvc:` one) restoring into a PVC that a later source names used to read `sources[0]`'s path; it now reads the matching source's own path (this one is the bug fix — the old answer was another volume's data).
...
The window also does **not** open while the objects Kopiur reads the repository from are missing: if the referenced `Repository` object, or the `SnapshotPolicy` a `fromPolicy` source names, doesn't exist yet, the restore parks in `Pending` with `ReferentAvailable=False reason=RestoreReferentMissing` and `status.waitStartedAt` stays unstamped (it re-checks every 15 s, so it un-parks on its own once you apply the missing object). Two cases deliberately **do** open the window unverified: a `snapshotRef` whose `Snapshot` row doesn't exist yet — that row is precisely what the window is waiting for, and `onMissingSnapshot` has to be able to fire for a ref that never appears — and a restore whose mover Job has already launched, which keeps the deadline it was dispatched with.
