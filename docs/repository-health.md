...
| **Mover permitted** | Reconcile | `MoverPermitted=False` | A privileged mover that wasn't opted in |
| **Stream-exec opt-in** | Reconcile | `MoverPermitted=False` (reason `StreamExecNotPermitted`) | A `stream` source used in a namespace that hasn't opted in via the `stream-exec-movers` annotation |
...
| **Security-context resolution** | `Snapshot`/`Restore` reconcile | `SecurityContextResolved=False` (reason `InheritSourceMissing`) | A genuine hold, not advisory: `inheritSecurityContextFrom` resolved no securityContext and no fallback `runAsUser` is pinned — parks at `Pending` and clears once the workload returns |
