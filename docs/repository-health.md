...
| **Stream-exec opt-in** | Admission / reconcile | `MoverPermitted=False` (reason `StreamExecNotPermitted`) | A `stream` source used in a namespace that hasn't opted in via the `stream-exec-movers` annotation |
