...
| Claim `Failed`, `reason=SourcePathAmbiguous` | The `SnapshotPolicy` names no single volume for this PVC: its selector sources disagree on `sourcePathStrategy`/`sourcePathOverride`, or they share one `sourcePathOverride` so no derivation can tell the members apart. Kopiur fails closed rather than restoring an arbitrary member. | Set `source.fromPolicy.sourcePath` on the `Restore` to the member you want (e.g. `/pvc/postgres-data`), or make the policy's selector sources agree. See [Restores → `sourcePath`](restores.md#sourcepath--which-volume-of-a-multi-pvc-policy-to-read). |
