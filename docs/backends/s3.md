...
- **`create.enabled`** at `true` initializes the repository if it doesn't exist. At `false`, which is the default, the repository must already exist. `create.encryption`, `create.splitter`, and `create.hash` are fixed forever at creation. See [repository creation](../repositories.md#encryption-and-repository-creation).
