...
| `kopiur_snapshot_waiting_for_slot`                | gauge       | `repository_kind`, `repository`, `namespace`, `name` | store-backed observable: Snapshots queued behind their repository's mover-Job concurrency cap (`RepositorySlotAvailable=False`). Deliberately a SEPARATE series from `kopiur_snapshot_gated` — a not-Ready repository is a fault to investigate, a full pool is the cap doing its job, and conflating them would make a correctly-throttled fleet indistinguishable from a broken one on the same alert. One series per queued CR (hence the singular metric name), so `sum by (repository)` is the live queue depth; it drains to **absence** rather than lingering at 0, and terminal-phase Snapshots are excluded by design so a stranded `False` condition can never hold the gauge on. **`namespace`/`name` are the SNAPSHOT's** — unlike the repository-family metrics below, whose `namespace` is the repository's own; join the two families on `repository`, never on `namespace`. `repository_kind`/`repository` come from the park-time `status.resolved.repository` pin (unpinned → the `"unknown"` bucket). Drives the `KopiurSnapshotWaitingForSlot` alert (per-series, `for: 30m`) and the "Mover-slot queue depth" dashboard panel |
...
| `kopiur_repo_catalog_coverage`                 | gauge       | `namespace`, `name`, `state` (`complete`/`capped`/`partial`) | store-backed: one series per repository, valued 1, labeled with the `state` matching `status.catalog.coverage` as of the last scan (#476); absent until the first scan, and gone when the repository is deleted (no 0-valued series). `complete` = the listing covered every snapshot; `capped` = only the newest materialization window (≤1,000) was materialized, but stale discovered rows are still expired; `partial` = capped **and** the snapshot-ID membership was unavailable (a mover image older than the controller, or a repository beyond the ~86k ID-list ceiling), so rows whose snapshots were deleted repository-side are **not** expired and `kopiur_repo_discovered_snapshots` can only grow until `catalog.retain` bounds it. Alert on `kopiur_repo_catalog_coverage{state="partial"} == 1` |
